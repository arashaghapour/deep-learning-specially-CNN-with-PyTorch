"""
The code here and coment because I write code and test it with small 
dataset and the train in larg batches in kaggle and download model then run predict in
local muchine so I dont need train code
...
#                                writer=writer)
#             experiment += 1
model_1, tranform = model_builder.create_b2_model(device=device)
model_builder.adjust_num_classes(model=model_1, num_class=3, device=device)
utils.load_model(model=model_1, model_path="./models/model10.pth")
predict.predict(model=model_1,
                image_pathes=test_data,
                num_pic=5,
                transform=tranform,
                classes=data_setup.imagefolder_without_transform(data_path=test_data).classes)