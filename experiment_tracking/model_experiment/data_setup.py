...
def fake_data(transform, batch_size: int):
    ...
    fake_test_dataset = FakeData(
    size=10,               
    image_size=(3, 224, 224),
    num_classes=101,        
    transform=transform
    )
    fake_train_dataloader = DataLoader(dataset=fake_train_dataset,
                                 batch_size=batch_size,
                                 shuffle=True)
    
    fake_test_dataloader = DataLoader(dataset=fake_test_dataset,
                                 batch_size=batch_size)
    return fake_train_dataloader, fake_test_dataloader, 101
